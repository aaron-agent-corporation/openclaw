...
with the official plugin (currently `@openai/codex` `0.154.0-alpha.11`):
...
The plugin manages stable Codex app-server `0.154.0-alpha.11`. Explicit custom
...
This preview path uses the pinned Codex `0.154.0-alpha.11` app-server.
...
<Note>
The current bundled harness is `@openai/codex` `0.154.0-alpha.11`. A live `model/list`
probe against the official `0.154.0-alpha.11` app-server, using an isolated,
...
| `codex-auto-review` | text, image      | low, medium, high, xhigh, max        |

The probe marked `gpt-5.4`, `gpt-5.4-mini`, and `codex-auto-review` as hidden. This snapshot does not
