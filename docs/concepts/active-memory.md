...
| `config.queryMode`           | `"message" \| "recent" \| "full" \| "accordion-aware"`                                               | Controls how much conversation the blocking memory sub-agent sees; `accordion-aware` also auto-expands a strongly-matched collapsed topic back into context for the current turn                                                                         |
