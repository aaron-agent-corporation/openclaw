...
/** Lists agent ids that pin the given auth profile id. */
export function listAgentsPinningAuthProfile(params: {
  cfg: OpenClawConfig;
  profileId: string;
}): string[] {
  const target = normalizeOptionalString(params.profileId);
  if (!target) {
    return [];
  }
  ...
  for (const entry of listAgentEntries(params.cfg)) {
    ...
    for (const profileId of Object.values(profiles)) {
      if (normalizeOptionalString(profileId) === target) {
        pinnedBy.push(entry.id);
        break;
      }
    }
  }
  return pinnedBy.sort((a, b) => a.localeCompare(b));
}

/** Builds provider → agent-id lists for every agent auth pin in config. */
