  ...
  implements Omit<AgentsState, "agentsLoading" | "agentsError">
{
  ...
  private async finishAuthSubscription(
    ...
    const client = this.client;
    const generation = this.requestGeneration;
    let after: ModelAuthStatusResult | null = null;
    if (client) {
      try {
    ...
    if (!client || !this.isCurrentRequest(client, generation, agentId)) {
      return;
    }
    if (after) {
      ...
      if (pin) {
        ...
        this.modelAuthStatusError = t("agents.overview.authPinSelectAfterSetup");
      }
    } else {
      this.modelAuthStatusError = t("agents.overview.authPinSelectAfterSetup");
    }
