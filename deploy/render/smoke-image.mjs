...
const info = JSON.parse(docker("run", "--rm", "--entrypoint", "node", image, "-e", `
...
const codexCli = docker("run", "--rm", "--entrypoint", "node", image,
  "/app/extensions/codex/node_modules/@openai/codex/bin/codex.js", "--version");
assert.equal(codexCli, `codex-cli ${info.codexVersion}`, "Managed Codex binary and package versions must match");
...
try {
  docker("run", "--detach", "--name", name, "--publish", "127.0.0.1::18789",
    "--env", "OPENCLAW_STATE_DIR=/tmp/openclaw-smoke",
    "--env", "OPENCLAW_CONFIG_PATH=/tmp/openclaw-smoke/openclaw.json",
    "--env", `OPENCLAW_GATEWAY_TOKEN=${token}`, image, "sh", "-c", initialize);
  ...
  while (Date.now() < deadline) {
    assert.equal(docker("inspect", "--format", "{{.State.Running}}", name), "true", "Gateway exited");
    const response = await fetch(`${base}/readyz`, { signal: AbortSignal.timeout(3000) }).catch(() => null);
    if (response?.ok) {
      ready = true;
      break;
    }
  ...
  try { console.error(docker("logs", "--tail", "100", name).replaceAll(token, "[smoke-token]")); } catch {}
  throw error;
} finally {
