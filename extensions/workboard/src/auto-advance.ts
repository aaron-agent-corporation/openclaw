...
export function createWorkboardAutoAdvanceService(params: {
  ...
  const startPass = (): void => {
    ...
    // Store retirement waits for the whole pass, including launch cleanup,
    // while stop invalidates the gate before any more workers can start.
    inFlight = params.store
      .runOperation(() => runPass(owner))
      .finally(() => {
        inFlight = undefined;
